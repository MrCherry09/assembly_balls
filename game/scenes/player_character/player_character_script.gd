...
## How quickly the body turns to face movement (detached free-look camera).
@export var body_turn_speed: float = 12.0
## How quickly the body turns to face the camera while aiming.
...
func _update_model_visuals(delta: float) -> void:
	...
	if cam_holder.is_aiming:
		# Snap body to aim direction with minimal lag
		body_yaw = lerp_angle(body_yaw, cam_holder.get_look_yaw(), clampf(aim_body_turn_speed * delta, 0.0, 1.0))
	else:
		# Detached free-look: only turn the body when there is move input
